      ...
      <template v-else>
        ...
        <template v-else-if="cell.type === 'buttonToggle'">
          ...
          <template v-if="cell.data.type === 'list'">
            ...
            >
              <li
                v-for="(l, index) in cell.data.listData.elements"
                :key="index"
              >
                {{ l }}
              </li>
        ...
        <template v-else-if="cell.type === 'number'">
          <input
            ...
            :value="cell.data.value"
            @change="handleChange(cell, $event)"
          />
        </template>
        ...
        <template v-else-if="cell.type === 'html'">
          <template v-html="cell.data"></template>
        </template>
        ...
        <template v-else-if="cell.type === 'unitSearch'">
          <div class="input-group input-group-sm add-unit-container">
            <UnitSearch
              :list="cell.data.list"
              @select="cell.data.value"
              @enterPress="handleClick(cell, $event)"
            />
            <button
              class="btn btn-sm btn-primary"
              :disabled="!cell.data.value"
              @click="handleClick(cell, cell.data.value)"
            >
              Add Unit
            </button>
          </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    handleChange: _.debounce(function (
      ...
      if (cell.change) {
        if (event) {
          cell.change((event.target as HTMLInputElement).value);
        } else {
          cell.change(cell.data.value);
        }
      }
    },
    500),
    handleClick: _.debounce(function (this: any, cell: iTableCell, data: any) {
      if (cell.click) {
        cell.click(data);
      }
    }, 500),
