...
export class RelicPlanner {
  ...
  constructor(
    ...
    const savedData: any[] = JSON.parse(
      window.localStorage.getItem("relicCalculatorData") ?? "[]"
    );
    const match = savedData.find((x) => x.id === id);
    if (match) {
      challengeGear.forEach((x) => {
        const exists = match.challengeGear.find((challengeGear: any) =>
          _.isEqual(x.ids, challengeGear.ids)
        );
        if (exists) {
      ...
      storeGear.forEach((x) => {
        const exists = match.storeGear.find((storeGear: any) =>
          _.isEqual(x.ids, storeGear.ids)
        );
        if (exists) {
  ...
  private get totalRelicMats() {
    return relicMapping[this._id].amountNeeded([
      { level: 0, target: this.relicTarget },
    ]);
  }
  public get remainingRelicPieces() {
    ...
    return Math.max(
      0,
      round2Decimals(this.totalRelicMats - challengesPieces - storePieces)
    );
  }
  ...
  public get remainingGear() {
    return round2Decimals(
      (this.relicPiecesPerDay / this._farmData.relicSalvage) *
        this._farmData.amountForRelicPiece
    );
  }
  ...
  public get totalEnergy() {
    return round2Decimals(
      this._farmData.energyPerNode * this.remainingGearPerDay
    );
  }
...
class StoreGear {
  ...
  public get relicPiecesPerDay() {
    return round2Decimals(
      this._purchases * this._gearPerPurchase * this._percentOfRelicPiece
    );
  }
  public get relicPiecesTotal() {
    return round2Decimals(this.relicPiecesPerDay * this.timeline);
...
const carbonite_circuit_board = savedData.find(
  (x) => x.id === "carbonite_circuit_board"
);
const bronzium_wiring = savedData.find((x) => x.id === "bronzium_wiring");
const chromium_transistor = savedData.find(
  (x) => x.id === "chromium_transistor"
);
const aurodium_heatsink = savedData.find((x) => x.id === "aurodium_heatsink");
const electrium_conductor = savedData.find(
  (x) => x.id === "electrium_conductor"
);
...
export const tableData: RelicPlanner[] = [
  new RelicPlanner(
