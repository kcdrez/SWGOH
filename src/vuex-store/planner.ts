...
import { PlannerItem, ConfigType, UpdateItem } from "../types/planner";
...
const store = {
  ...
  getters: {
    ...
    unitList(state: State, getters: any, rootState: RootState) {
      return Object.keys(state.plannerConfig).map((id: string) => {
        const match = rootState.unit.unitList.find(x => x.id === id);
        return {
          id,
  ...
  mutations: {
    ...
    UPDATE_PLANNER(state: State, payload: ConfigType) {
      ...
      window.localStorage.setItem("generalPlanner", JSON.stringify(state.plannerConfig));
    },
    UPDATE_PLANNER_ITEM(state: State, { unitId, type, value }: UpdateItem) {
      if (!state.plannerConfig[unitId]) {
        state.plannerConfig[unitId] = { gear: { target: 0 }, relic: { target: 0 } }
      }
      if (type === "gear") {
      ...
      state.plannerConfig[unitId][type].target = value;
      window.localStorage.setItem("generalPlanner", JSON.stringify(state.plannerConfig));
    }
  },
  actions: {
    async initialize({ commit, dispatch }: ActionCtx) {
      ...
      await Promise.all(Object.keys(data).map(id => dispatch("unit/getUnit", id, { root: true })));
      commit("UPDATE_PLANNER", data)
    },
    addUnit({ commit }: ActionCtx, { unitId, gearTarget, relicTarget }: { unitId: string, gearTarget: number, relicTarget: number }) {
      commit("UPDATE_PLANNER_ITEM", { type: 'gear', value: gearTarget, unitId });
      commit("UPDATE_PLANNER_ITEM", { type: 'relic', value: relicTarget, unitId });
    },
